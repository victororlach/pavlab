...
int main(int argc, char *argv[])
{
    ...
    if (argv[2] && (fopen("data_ampliacion.txt", "w") != NULL))
    {
    ...
    float *left_samples = malloc(N / 2 * sizeof(int16_t));
    float *right_samples = malloc(N / 2 * sizeof(int16_t));
    ...
    while (lee_wave(buffer, sizeof(*buffer), N, fpWave) == N)
    {
        ...
        if (trm == 0)
        {
            ...
            if (stereo)
            {
                ...
                {
                    left_samples[i] = buffer[2 * i] / (float)(1 << 15);
                    right_samples[i] = buffer[2 * i + 1] / (float)(1 << 15);
                }
            }
            else
            {
            ...
            if (argv[2]) //.txt printing
            {
                // pass different arguments to compute_hamming_power() depending on stereo
                if (stereo)
                {
        ...
        else
        {
            ...
            while (iteration < 2)
            {
                ...
                // pointers to store channel powers
                float left_channel_power;
                float right_channel_power;

                if (argv[2]) // if we want to print to .txt file
                {
                    // pass different arguments to compute_hamming_power() depending on stereo
                    if (stereo)
                    {
